...
test("camera, selection and gestures share one ownership model", async ({
  page,
}) => {
  ...
  const tempelExplore = await snapshot(page);
  expect(tempelExplore.camera?.selectedBodyId).toBe("tempel-1");
  expect(tempelExplore.camera?.targetBodyId).toBe("tempel-1");
  expect(tempelExplore.camera?.distanceToTarget).toBeGreaterThanOrEqual(
    tempelExplore.camera?.minimumDistance ?? 0,
  );
  ...
  expect(tempelScientific.camera?.targetBodyId).toBe("tempel-1");
  expect(tempelScientific.camera?.distanceToTarget).toBeGreaterThanOrEqual(
    tempelScientific.camera?.minimumDistance ?? 0,
  );
  expect(tempelScientific.camera?.distanceToTarget).toBeLessThan(
    tempelExplore.camera?.distanceToTarget ?? Number.POSITIVE_INFINITY,
  );
  ...
  expect(tempelExploreAgain.camera?.distanceToTarget).toBeGreaterThan(
    tempelScientific.camera?.distanceToTarget ?? 0,
  );
