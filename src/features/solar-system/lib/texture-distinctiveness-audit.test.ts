  ...
  return JSON.parse(
    readFileSync(
      join(
        process.cwd(),
        "test-artifacts",
        "texture-distinctiveness-audit.json",
      ),
      "utf8",
    ),
...
describe("procedural texture distinctiveness audit", () => {
  ...
  it("rejects near-duplicate procedural surfaces while retaining GPU review", () => {
    const result = artifact();
