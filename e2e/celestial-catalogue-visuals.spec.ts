...
import type { CelestialSceneProbeSnapshot } from "../src/features/solar-system/components/celestial-scene-probe";
...
async function celestialProbeSnapshot(
  page: Page,
): Promise<CelestialSceneProbeSnapshot | null> {
  return page.evaluate(() => window.__HELIOS_CELESTIAL_SCENE_TEST__ ?? null);
}

async function waitForCelestialProbe(page: Page) {
  await expect
    .poll(async () => Boolean(await celestialProbeSnapshot(page)), {
      timeout: 45_000,
    })
    .toBe(true);
}
...
async function waitForScene(page: Page) {
  ...
  await waitForCelestialProbe(page);
}
...
test("catalogue completeness and object-local surface contracts", async ({
  ...
  >) {
    ...
    for (let pageNumber = 1; pageNumber <= expected.pages; pageNumber += 1) {
      ...
      const probe = await celestialProbeSnapshot(page);
      ...
      for (const id of scene?.catalogue.bodyIds ?? []) {
        const body = probe?.catalogueBodies[id];
    ...
    if (mode === "moons") {
      ...
      await expect
        .poll(
          async () =>
            (await celestialProbeSnapshot(page))?.catalogueBodies[
              "moon-mars-phobos"
...
test("featured moon selection preserves tidal orientation, orbit and camera ownership", async ({
  ...
  for (const [parent, name, id] of representatives) {
    ...
    await expect
      .poll(
        async () =>
          (await celestialProbeSnapshot(page))?.catalogueBodies[id]
            ?.rotationKind,
      )
      .toBe("tidally-locked");
    await expect
...
test("asteroid and dwarf-world silhouettes, rings and surfaces remain distinct", async ({
  ...
  let probe = await celestialProbeSnapshot(page);
  expect(probe?.catalogueBodies.ceres.geometryKind).toBe("sphere");
  expect(probe?.catalogueBodies.vesta.geometryKind).toBe("irregular");
  expect(probe?.catalogueBodies.pallas.geometryKind).toBe("irregular");
  expect(probe?.catalogueBodies.hygiea.geometryKind).toBe("ellipsoid");
  await openCatalogue(page, "dwarf-kuiper");
  probe = await celestialProbeSnapshot(page);
  expect(probe?.catalogueBodies.haumea.ringMounted).toBe(true);
  expect(probe?.catalogueBodies.quaoar.ringMounted).toBe(true);
  expect(probe?.catalogueBodies.pluto.atmosphereMounted).toBe(true);
  await expectCleanRuntime(audit);
});
...
test("dwarf systems preserve barycentric, parent-local and independent selection context", async ({
  ...
  const dwarfProbe = await celestialProbeSnapshot(page);
  expect(
...
test("comet dynamics use shared time, local tail anchors and anti-solar direction", async ({
  ...
  for (const [name, id] of [
    ...
    await expect
      .poll(
        async () => (await celestialProbeSnapshot(page))?.orbits[id]?.emphasis,
      )
      .toBe("selected");
    const orbit = (await celestialProbeSnapshot(page))?.orbits[id];
    expect(orbit?.closed).toBe(true);
...
test("planetary rings preserve counts, open arcs, equatorial transforms and focus context", async ({
  ...
  for (const [name, id, bands, arcs] of expectations) {
    ...
    await expect
      .poll(
        async () => (await celestialProbeSnapshot(page))?.rings[id]?.bandCount,
      )
      .toBe(bands);
    const ring = (await celestialProbeSnapshot(page))?.rings[id];
    ...
    if (id === "saturn") {
      await expect
        .poll(
          async () =>
            (await celestialProbeSnapshot(page))?.rings.saturn?.textureReady,
        )
        .toBe(true);
    }
    await expect
...
test("orbit identity remains stable through select, clear and visibility changes", async ({
  ...
  for (let index = 0; index < ids.length; index += 1) {
    const id = ids[index];
    const before = (await celestialProbeSnapshot(page))?.orbits[id];
    ...
    await expect
      ...
      .toBe("selected");
    const selected = (await celestialProbeSnapshot(page))?.orbits[id];
    ...
    await expect
      ...
      .toBe("hidden");
    const hidden = (await celestialProbeSnapshot(page))?.orbits[id];
    ...
    await expect
      ...
      .not.toBe("hidden");
    const restored = (await celestialProbeSnapshot(page))?.orbits[id];
    ...
    if ((await clear.count()) > 0) {
      ...
      await expect
        .poll(
          async () =>
            (await celestialProbeSnapshot(page))?.orbits[id]?.emphasis,
        )
        .toBe("context");
    }
    const cleared = (await celestialProbeSnapshot(page))?.orbits[id];
...
test("deterministic rotation returns to timestamp A and leaves unknown bodies fixed", async ({
  ...
  // for the production eight-planet texture check that it intentionally omits.
  await waitForScene(page);
  ...
  const readAngles = async (timestamp: string) => {
    ...
    await expect
      .poll(
        async () =>
          (await celestialProbeSnapshot(page))?.catalogueBodies["67p"]
            .rotationAngle ?? null,
      )
      .not.toBeNull();
    const bodies = (await celestialProbeSnapshot(page))?.catalogueBodies;
    return {
