...
export interface CelestialCatalogueBodyProbe {
...
export interface CelestialCometProbe {
...
export interface CelestialDwarfOrbitPlaneProbe {
...
export interface CelestialOrbitProbe {
...
export interface CelestialRingProbe {
...
export interface CelestialSceneProbeSnapshot {
  readonly catalogueBodies: Readonly<
    Record<string, CelestialCatalogueBodyProbe>
  >;
  readonly comets: Readonly<Record<string, CelestialCometProbe>>;
  readonly dwarfOrbitPlanes: Readonly<
    Record<string, CelestialDwarfOrbitPlaneProbe>
  >;
  readonly orbits: Readonly<Record<string, CelestialOrbitProbe>>;
  readonly rings: Readonly<Record<string, CelestialRingProbe>>;
}

declare global {
  interface Window {
    __HELIOS_ENABLE_SCENE_TEST__?: boolean;
    __HELIOS_CELESTIAL_SCENE_TEST__?: CelestialSceneProbeSnapshot;
  }
}
...
function ActiveCelestialSceneProbe() {
  const scene = useThree((state) => state.scene);

  useFrame(() => {
    const catalogueBodies: Record<string, CelestialCatalogueBodyProbe> = {};
    const comets: Record<string, CelestialCometProbe> = {};
    const dwarfOrbitPlanes: Record<string, CelestialDwarfOrbitPlaneProbe> = {};
    const orbits: Record<string, CelestialOrbitProbe> = {};
    const rings: Record<string, CelestialRingProbe> = {};
    ...
    scene.traverse((object) => {
      ...
      if (orbitId) {
        ...
        ) as CelestialOrbitProbe["emphasis"];
        orbits[orbitId] = {
    ...
    window.__HELIOS_CELESTIAL_SCENE_TEST__ = {
      catalogueBodies,
      comets,
      dwarfOrbitPlanes,
      orbits,
...
export function CelestialSceneProbe() {
  ...
  return enabled ? <ActiveCelestialSceneProbe /> : null;
}
