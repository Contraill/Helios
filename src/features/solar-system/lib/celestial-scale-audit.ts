...
export type CelestialScaleAuditStatus = "pass" | "review" | "fail";

export interface CelestialScalePair {
  readonly exploration: number;
  readonly scientific: number;
}

export interface CelestialScaleAuditRow {
  ...
  readonly visualAmplification: CelestialScalePair;
  readonly parentDistance: CelestialScalePair & { readonly physicalKm: number };
  readonly orbitExtent: CelestialScalePair;
  readonly focusRadius: CelestialScalePair;
  readonly geometryBounds: CelestialScalePair;
  readonly systemExtent: CelestialScalePair;
  readonly representationStatus: CelestialScaleAuditStatus;
  readonly flags: readonly string[];
}

export interface CelestialScaleAuditArtifact {
  readonly schemaVersion: 1;
  readonly sourceModule: string;
  readonly bodies: readonly CelestialScaleAuditRow[];
  readonly summary: {
...
function statusFor(flags: readonly string[]): CelestialScaleAuditStatus {
...
function featuredMoonRows(): CelestialScaleAuditRow[] {
...
function dwarfSatelliteRows(): CelestialScaleAuditRow[] {
...
function extendedBodyRows(): CelestialScaleAuditRow[] {
...
export function createCelestialScaleAudit(): CelestialScaleAuditArtifact {
  ...
  if (bodies.length !== visualRegistryIds.length) {
    throw new Error(
      `Celestial scale audit expected ${visualRegistryIds.length} bodies, received ${bodies.length}.`,
    );
  }
  ...
  return Object.freeze({
    schemaVersion: 1,
    sourceModule: "celestial-scale-audit.ts",
    bodies,
    summary,
  });
}

export const CELESTIAL_SCALE_AUDIT = createCelestialScaleAudit();
