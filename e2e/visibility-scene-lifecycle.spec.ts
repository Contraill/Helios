...
async function keepPrimarySceneAssetsBounded(page: Page) {
...
test("visibility controls keep scene ownership stable while state remains explicit", async ({
  page,
}) => {
  test.setTimeout(180_000);
  await page.addInitScript(() => localStorage.clear());
  await page.emulateMedia({ reducedMotion: "reduce" });
  await keepPrimarySceneAssetsBounded(page);
...
test.describe("compact visibility layout", () => {
