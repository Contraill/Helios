...
import {
  CELESTIAL_SCALE_AUDIT,
  type CelestialScaleAuditArtifact,
} from "./celestial-scale-audit";
...
describe("celestial scale and framing audit", () => {
  it("covers every visual body exactly once", () => {
    expect(CELESTIAL_SCALE_AUDIT.bodies).toHaveLength(48);
    expect(
      new Set(CELESTIAL_SCALE_AUDIT.bodies.map((body) => body.bodyId)),
  ...
  it("keeps all measured values finite and scientific radii unamplified", () => {
    ...
    expect(CELESTIAL_SCALE_AUDIT.summary.fail).toBe(0);
  });
  ...
  it("frames dwarf parent systems from their actual satellite extent", () => {
    for (const parentId of [
      ...
      const row = CELESTIAL_SCALE_AUDIT.bodies.find(
  ...
  it("keeps the committed machine-readable artifact synchronized", () => {
    const artifact = JSON.parse(
      readFileSync(
        join(process.cwd(), "test-artifacts", "celestial-scale-audit.json"),
        "utf8",
      ),
    ) as CelestialScaleAuditArtifact;
    expect(rounded(artifact)).toEqual(rounded(CELESTIAL_SCALE_AUDIT));
  });
});
